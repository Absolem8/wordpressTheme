...
if (!function_exists('add_scripts')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_scripts() { // добавление скриптов
	    ...
	    wp_enqueue_script('jquery3','//code.jquery.com/jquery-3.3.1.min.js','','',true); // добавляем свой
	    wp_enqueue_script('bootstrap', get_template_directory_uri().'/js/bootstrap.min.js','','',true); // бутстрап
	    wp_enqueue_script('gijgo', '//unpkg.com/gijgo@1.9.13/js/gijgo.min.js','','',true); // и скрипты шаблона
...
if (!function_exists('add_styles')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_styles() { // добавление стилей
		...
		wp_enqueue_style( 'gijgo', '//unpkg.com/gijgo@1.9.13/css/gijgo.min.css' ); // основные стили шаблона
	}
}
...
function add_menuclass($ulclass) {
	return preg_replace('/<a /', '<a class="nav-link text-light"', $ulclass);
}
